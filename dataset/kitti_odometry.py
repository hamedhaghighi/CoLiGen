...
class KITTIOdometry(torch.utils.data.Dataset):
    ...
    def load_datalist(self):
        ...
        if self.has_label:
            self.label_list = [
                d.replace("velodyne", "labels").replace(
                    "bin" if self.is_raw else "npy", "label" if self.is_raw else "png"
                )
                for d in self.datalist
            ]
        if self.has_rgb:
            self.rgb_list = [
                d.replace("velodyne", "image_2").replace(
                    "bin" if self.is_raw else "npy", "png"
                )
                for d in self.datalist
            ]
        self.tag_list = (
            [d.replace("velodyne", "tag").replace("bin", "tag") for d in self.datalist]
            if self.name == "semanticPOSS" and self.is_raw
            else None
        )
    ...
    def transform(self, out):
        ...
        if self.finesize is not None:
            i, j, h, w = transforms.RandomCrop.get_params(
                torch.zeros(1, self.shape[0], self.shape[1]),
                output_size=(self.finesize, self.finesize),
            )
    ...
    def __getitem__(self, index):
        ...
        if not self.is_raw:
            ...
            if self.has_label:
                ...
                points = np.concatenate(
                    [points, sem_label.astype("float32")[..., None]], axis=-1
                )
            if self.has_rgb:
            ...
            if self.has_label:
                ...
                if self.name in ["kitti", "carla", "semanticPOSS", "wads"]:
                    ...
                    sem_label = _map(
                        _map(sem_label, self.DATA.learning_map),
                        self.DATA.m_learning_map,
                    )
                ...
                point_cloud = np.concatenate(
                    [point_cloud, sem_label.astype("float32")[:, None]], axis=1
                )
            if self.has_rgb:
                rgb_path = self.rgb_list[index]
                ...
                point_cloud = np.concatenate(
                    [point_cloud, rgb.astype("float32")], axis=1
                )
            H, W = self.DATA.height, self.DATA.width
            fov_up, fov_down = self.DATA.fov_up, self.DATA.fov_down

            tag = (
                np.fromfile(self.tag_list[index], dtype=np.bool)
                if self.tag_list is not None
                else None
            )
            points, _ = point_cloud_to_xyz_image(
                point_cloud,
                H=H,
                W=W,
                fov_down=fov_down,
        ...
        if "label" in self.modality:
            out["label"] = (
                points[..., [4]] / (10.0 if self.name == "semanticPOSS" else 19.0)
                if self.norm_label
                else points[..., [4]]
            )
            # if self.name == 'carla' :
            ...
            if self.name == "carla" and self.is_ref_semposs:
                out["lwo"] = _map(
                    out["lwo"].astype("int"), self.DATA.kitti_to_POSS_map
                ).astype(np.float32)
        if self.has_rgb:
