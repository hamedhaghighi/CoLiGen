...
def get_dataset(
    ...
    if dataset_name in ["kitti", "carla", "synthlidar", "semanticPOSS", "wads"]:
        dataset = KITTIOdometry(
            ...
            finesize=(
                cfg.img_prop.finesize
                if (split == "train" and cfg.img_prop.finesize != -1)
                else None
            ),
...
def get_data_loader(
    ...
    ds_cfg_A = make_class_from_dict(
        yaml.safe_load(open(f"configs/dataset_cfg/{dataset_name_A}_cfg.yml", "r"))
    )
    ...
    if hasattr(cfg.dataset, "dataset_B") and two_dataset_enabled:
        ...
        ds_cfg_B = make_class_from_dict(
            yaml.safe_load(open(f"configs/dataset_cfg/{cfg_B.name}_cfg.yml", "r"))
        )
        dataset_B = get_dataset(
            cfg.dataset.dataset_B.name,
            cfg_B,
            ds_cfg_B,
            cfg_B.data_dir,
            split,
