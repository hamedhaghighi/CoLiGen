...
class SemanticKitti(Dataset):
    ...
    def __getitem__(self, index):
        ...
        self.empirical_max = (
            ...
            + 4 * self.sensor_img_stds[:, None, None]
        )
        self.empirical_min = (
            self.sensor_img_means[:, None, None]
            - 4 * self.sensor_img_stds[:, None, None]
        ...
        proj = (proj - self.empirical_min) / (self.empirical_max - self.empirical_min)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--total_samples", "-t", dest="total_samples", type=int, default=-1
    )
    parser.add_argument("--n_seq", "-s", dest="n_seq", type=int)
    parser.add_argument(
        "--data_dir",
        "-d",
        dest="data_dir",
    ...
    parser.add_argument("--have_label", "-hl", dest="have_label", action="store_true")
    ...
    for seq in seqs_list:
        ...
        n_sample = (
            min(len(scan_files), even_n_samples)
            if even_n_samples != -1
            else len(scan_files)
        )
    ...
    for scan_path in tqdm.tqdm(scan_files_list, total=len(scan_files_list)):
        ...
        proj_rgb = (
            np.transpose(scan.proj_points_rgb, (2, 0, 1)).astype("float32")
            if cfg["sensor"]["have_rgb"]
            else None
        )
        ...
        if have_label:
            ...
            proj_sem_label = np.expand_dims(
                np.array(scan.proj_sem_label, dtype=np.float32), axis=0
            )
            proj_inst_label = np.expand_dims(
                np.array(scan.proj_inst_label, dtype=np.float32), axis=0
            )
        ...
        if proj_rgb is not None and have_label:
            ...
            proj = np.concatenate(
                [proj_xyz, proj_range, proj_remission, proj_mask, proj_rgb]
            )
        else:
        ...
        filename = (
            "seq_" + splited[-3] + "_velodyne_" + splited[-1].split(".")[0] + ".npy"
        )
        ...
        for i in range(5):
            ...
            min_max[i][0] = min(
                min_max[i][0], (proj[i : i + 1][proj_mask == 1.0]).min()
            )
            min_max[i][1] = max(
                min_max[i][1], (proj[i : i + 1][proj_mask == 1.0]).max()
            )
