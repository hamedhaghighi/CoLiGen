...
if __name__ == "__main__":
    ...
    for seq in tqdm.tqdm(range(sequences_num), total=sequences_num):
        ...
        if have_label:
            dest_velo_label_dir_seq = os.path.join(
                destdir, "sequences", sequence, "labels"
            )
            os.makedirs(dest_velo_label_dir_seq, exist_ok=True)

        num_frames = len(glob.glob(velo_dir + "/*"))
        n_frame_in_each_seq = (
            min(num_frames, sample_num // sequences_num)
            if sample_num != -1
            else num_frames
        )
        frames = (
            sorted(
                np.random.choice(
                    np.arange(num_frames), n_frame_in_each_seq, replace=False
                ).tolist()
            )
            if sample_num != -1
            else np.arange(num_frames).tolist()
        )
        dataset = pykitti.odometry(basedir, sequence, frames=frames)
        if have_label:
            velo_label_files = sorted(
                glob.glob(
        ...
        for pc, img, fr in tqdm.tqdm(
            zip(dataset.velo, dataset.cam2, frames), total=len(frames)
        ):
            ...
            pc_in_image = np.array(
                [
                    pc_in_image[0] / pc_in_image[2],
                    pc_in_image[1] / pc_in_image[2],
                    pc_in_image[2],
                ]
            )
            canvas_mask = (
            ...
            dest_velo_filename = os.path.join(
                dest_velo_dir_seq, "{:06d}.bin".format(fr)
            )
            pc_with_rgb.reshape(-1).tofile(dest_velo_filename)

            if label is not None:
                label = label[canvas_mask]
                dest_velo_label_filename = os.path.join(
                    dest_velo_label_dir_seq, "{:06d}.label".format(fr)
                )
