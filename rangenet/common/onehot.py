...
class oneHot(nn.Module):
    ...
    def onehot1dspatial(self, x):
        ...
        # scatter to onehot
        one_hot = torch.zeros((n, self.nclasses, b), device=self.device).scatter_(
            1, x.unsqueeze(1), 1
        )
    ...
    def onehot2dspatial(self, x):
        ...
        # scatter to onehot
        one_hot = torch.zeros((n, self.nclasses, h, w), device=self.device).scatter_(
            1, x.unsqueeze(1), 1
        )
...
if __name__ == "__main__":
    ...
    tensor = torch.arange(0, nclasses).to(device)  # [0,1,2,3,4,5]
    tensor = torch.cat(
        [tensor.unsqueeze(0), tensor.unsqueeze(0)]
