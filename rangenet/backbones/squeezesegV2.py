...
class Fire(nn.Module):
    def __init__(
        self, inplanes, squeeze_planes, expand1x1_planes, expand3x3_planes, bn_d=0.1
    ):
...
class Backbone(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.conv1b = nn.Sequential(
            nn.Conv2d(self.input_depth, 64, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(64, momentum=self.bn_d),
        )
        self.fire23 = nn.Sequential(
            nn.MaxPool2d(kernel_size=3, stride=[1, self.strides[1]], padding=1),
        ...
        self.fire45 = nn.Sequential(
            nn.MaxPool2d(kernel_size=3, stride=[1, self.strides[2]], padding=1),
            Fire(128, 32, 128, 128, bn_d=self.bn_d),
            Fire(256, 32, 128, 128, bn_d=self.bn_d),
        )
        self.fire6789 = nn.Sequential(
            nn.MaxPool2d(kernel_size=3, stride=[1, self.strides[3]], padding=1),
