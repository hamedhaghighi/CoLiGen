...
class Trainer:
    def __init__(self, ARCH, DATA, datadir, logdir, path=None):
        ...
        # get the data
        parserPath = os.path.join(
            booger.TRAIN_PATH, "tasks", "semantic", "dataset", "kitti", "parser.py"
        )
        ...
        with torch.no_grad():
            dataset_name = self.DATA["name"]
            self.model = Segmentator(cfg_path=self.path, dataset_name=dataset_name)
        ...
        if self.ARCH["backbone"]["train"]:
            ...
            self.train_dicts.append({"params": self.model_single.backbone.parameters()})
        if self.ARCH["decoder"]["train"]:
            self.lr_group_names.append("decoder_lr")
            self.train_dicts.append({"params": self.model_single.decoder.parameters()})
        ...
        with torch.no_grad():
            ...
            print(
                ...
                "IoU avg {iou.avg:.3f}".format(
                    batch_time=batch_time, loss=losses, acc=acc, iou=iou
                )
            )
