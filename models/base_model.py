...
class BaseModel(ABC):
    ...
    def __init__(self, opt):
        ...
        self.save_dir = os.path.join(
            opt.training.checkpoints_dir, opt.training.name
    ...
    def calc_supervised_metrics(self, no_inv, lidar_A, lidar_B):
        ...
        self.depth_accuracies = {
            "depth/" + k: v.mean().item() for k, v in accuracies.items()
        }
        ...
        if "reflectance" in self.opt.model.modality_B:
            ...
            self.reflectance_errors = {
                "reflectance/" + k: v.mean().item() for k, v in errors.items()
            }
            self.reflectance_ssim = self.crterionSSIM(
                self.real_reflectance,
                self.synth_reflectance,
                torch.ones_like(self.real_reflectance),
            )
