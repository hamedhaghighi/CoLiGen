...
class GcGANModel(BaseModel):
    def __init__(self, opt, lidar_A, lidar_B):
        ...
        if self.isTrain:
            ...
            self.model_names = (
                ["G_AB", "G_gc_AB"] if "cross" in opt_m.name else ["G_AB"]
            )
        ...
        same_kernel_size = (
            opt.dataset.dataset_A.img_prop.width
            == opt.dataset.dataset_A.img_prop.height
        )

        self.netG_AB = networks.define_G(
            input_nc_G,
            output_nc_G,
            opt_m.ngf,
            opt_m.netG,
            ...
            self.gpu_ids,
            opt_m.out_ch,
            same_kernel_size=same_kernel_size,
        )
        if "cross" in opt_m.name:
            self.netG_gc_AB = networks.define_G(
                input_nc_G,
                output_nc_G,
                opt_m.ngf,
                opt_m.netG,
        ...
        if self.isTrain:
            ...
            self.netD_gc_B = networks.define_D(
                input_nc_D,
                opt_m.ndf,
                opt_m.netD,
                opt_m.n_layers_D,
        ...
        if self.isTrain:
            ...
            if "cross" in opt_m.name:
                self.optimizer_G = torch.optim.Adam(
                    itertools.chain(
                        self.netG_AB.parameters(), self.netG_gc_AB.parameters()
                    ),
                    lr=opt_t.lr,
                    betas=(opt_t.beta1, 0.999),
                )
            elif "share" in opt_m.name:
                self.optimizer_G = torch.optim.Adam(
                    self.netG_AB.parameters(), lr=opt_t.lr, betas=(opt_t.beta1, 0.999)
                )
    ...
    def forward(self):
        ...
        if self.opt.model.geometry == "rot":
            ...
            raise ValueError(
                "Geometry transformation function [%s] not recognized."
                % self.opt.geometry
            )
    ...
    def backward_D_B(self):
        ...
        loss_D_B = self.backward_D_basic(
            self.netD_B, self.real_B, fake_B, self.netD_gc_B, self.real_gc_B, fake_gc_B
        )
    ...
    def backward_G(self):
        ...
        if self.opt.model.identity > 0:
            ...
            loss_idt = (
                self.criterionIdt(idt_A, self.real_B)
                * self.opt.model.lambda_AB
                * self.opt.model.identity
            )
            _, idt_gc_A = self.netG_gc_AB(self.real_gc_B_mod_A)
            loss_idt_gc = (
                self.criterionIdt(idt_gc_A, self.real_gc_B)
                * self.opt.model.lambda_AB
                * self.opt.model.identity
            )
