...
def get_scheduler(optimizer, opt):
    ...
    if opt.lr_policy == "linear":
        ...
        def lambda_rule(epoch):
            lr_l = 1.0 - max(0, epoch - opt.epoch_decay) / float(
                opt.n_epochs - opt.epoch_decay + 1
            )
            return lr_l
...
def init_net(
    net, init_type="normal", init_gain=0.02, gpu_ids=[], initialize_weights=True
):
...
def define_G(
    ...
    if netG == "resnet_9blocks":
        net = ResnetGenerator(
            ...
            have_cond_modality=have_cond_mod,
        )
...
class ResnetGenerator(nn.Module):
    ...
    def forward(self, input, layers=[], encode_only=False, cond=None, cond_layer=None):
        ...
        if encode_only:
            ...
            return feat
        return disentangle_output(
            feat, self.out_ch, self.gumbel, self.out_modality
...
class UnetGenerator_2(nn.Module):
    ...
    def __init__(
        ...
        unet_block = UnetSkipConnectionBlock(
            ...
            norm_layer=norm_layer,
            innermost=True,
            same_size_kernel=same_kernel_size,
...
class UnetGenerator(nn.Module):
    ...
    def __init__(
        ...
        model += [
            nn.Conv2d(
                input_nc, ngf, kernel_size=k, stride=s, padding=(1, 1), bias=use_bias
            )
        ]
        for i in range(3):
            model += [
        ...
        model += [
            nn.Sequential(
                *[
                    uprelu,
                    nn.ConvTranspose2d(
                        ngf * 2, output_nc, kernel_size=k, stride=s, padding=(1, 1)
                    ),
                ]
            )
        ]
    ...
    def __init__(
        ...
        if outermost:
            ...
            upconv = nn.ConvTranspose2d(
                inner_nc * 2, outer_nc, kernel_size=k, stride=s, padding=(1, 1)
            )
            down = [downconv]
            up = [uprelu, upconv]
