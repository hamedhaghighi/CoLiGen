...
class Pix2Pix2ModalityModel(BaseModel):
    ...
    def __init__(self, opt, lidar):
        ...
        if self.isTrain:
            self.optimizer_G = torch.optim.Adam(
                self.netG.parameters(), lr=opt_t.lr, betas=(opt_t.beta1, 0.999)
            )
            self.optimizer_D = torch.optim.Adam(
                self.netD.parameters(), lr=opt_t.lr, betas=(opt_t.beta1, 0.999)
            )
    ...
    def calc_loss_G(self):
        ...
        self.loss_G = (
            self.loss_G_GAN * self.opt.model.lambda_LGAN
            + self.loss_G_L1 * self.opt.model.lambda_L1
            + self.loss_mask_bce * self.opt.model.lambda_mask
        )
